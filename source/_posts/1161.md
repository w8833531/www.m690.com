...
在关掉两台control plane node情况下，如下图：
...
结论：1.在关掉三台kubernetes cluster control plane node中的任意一台，整个cluster是可以正常工作的。被关掉了那台control plane node上的pod会迁移到其它 control plane node上（设置control plane node上可以部署pod)。2.在关掉三台中的两台control plane node的情况下，整个cluster不可用。
