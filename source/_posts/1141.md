...
我没有单独运行一个容器来跑mysql,然后做link。 因为本机上已经有mysql在跑了。如何从win10的容器来访问本机的mysql呢？应该使用本机的实际IP,如本机IP是172.18.199.49,那么django项目的setting.py中的mysql的HOST设置就应该是如下（记得在mysql中设置172.18.199.14的root访问权限 ：）：
```
DATABASES = {
    'default': {
